# Mico Interactive Anatomy Curriculum Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Turn Mico from a short MCQ path into a medically useful, Duolingo-paced anatomy course where students learn by rotating, zooming, identifying, and reasoning with real 3D structures.
...
**Files:**
- Create: `app/mico-lesson-types.ts`
- Create: `app/mico-curriculum.ts`
- Modify: `app/mico-data.ts`
- Test: `app/mico-curriculum.test.ts`

1. Write failing tests that reject activities missing a target/accepted answer and assert every lesson has 6+ activities.
2. Define `Activity`, `Lesson`, `Unit`, source/review metadata, and activity validation helpers in `mico-lesson-types.ts`.
3. Move current content into `mico-curriculum.ts`; expand Foundations and Heart first to the course map above without reusing the same question array.
4. Update `mico-data.ts` to export the curriculum-derived `units` and `allLessons` for compatibility.
...
**Files:**
- Create: `app/mico-activity-player.tsx`
- Create: `app/mico-activity-player.css`
- Create: `app/mico-activity-player.test.tsx`
- Modify: `app/mico-app.tsx`
- Modify: `app/mico-lesson-options.css`
- Modify: `app/mico-lesson-chrome.css`
...
**Files:**
- Create: `app/mico-progress-repository.ts`
- Create: `app/mico-progress-repository.test.ts`
- Modify: `app/mico-progress.ts`
- Modify: `docs/supabase/mico-schema.sql`
- Create: `docs/supabase/mico-sync-contract.md`

1. Write a contract test that local and Supabase implementations return the same progress shape.
2. Create a localStorage repository for guest progress and migrations from the current `MicoProgress` format.
...
- Modify: `app/mico-activity-player.tsx`
