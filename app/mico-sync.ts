...
export const applyLearningResult=(current:MicoProgress,result:LearningResult):MicoProgress=>({...current,xp:result.xp,hearts:result.hearts,dailyXp:result.daily_xp,completed:asStrings(result.completed_lessons),mastery:asRecord(result.mastery),unlimitedHeartsUntil:result.unlimited_hearts_until??undefined});
