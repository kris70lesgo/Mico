...
type Props = {
  ...
  onInteraction?: (sound: MicoSound) => void;
};
...
function ModelGuide({ organId }: { organId: string }) {
  ...
  return (
    <section className="mico-model-guide" aria-label="Structure guide">
      <div>
...
function AtlasLessonModel({
  ...
  if (!atlas)
    return (
      <div className="mico-real-model-loading">
...
export default function MicoActivityPlayer({
  activity,
  disabled,
  onAnswerChange,
  onInteraction,
  ...
  const optionList =
    ...
    activity.kind === "case-application"
      ...
      : undefined;
  return (
    <div className={`mico-activity mico-activity-${activity.kind}`}>
      <div className="mico-activity-brief">
        ...
        <>
          ...
          >
            ...
            {activity.kind === "type-label" && (
              <div className="mico-target-status">
                <Type size={16} />
                {activity.hint}
              </div>
            )}
      ...
      {activity.kind === "type-label" && (
        <label className="mico-answer-input">
      ...
      {activity.kind === "sequence-flow" && (
        <div className="mico-sequence">
          ...
          <div className="mico-sequence-bank">
            {remaining.map((step) => (
              <button
                key={step.id}
