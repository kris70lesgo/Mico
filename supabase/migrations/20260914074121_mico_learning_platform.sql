-- Mico's learner data. Every user-owned record is protected by RLS.
create schema if not exists private;

create table public.profiles (
  id uuid primary key references auth.users(id) on delete cascade,
  username text unique,
  display_name text not null default 'Mico learner',
  avatar_url text,
...
begin
  ...
  values (
    new.id,
    coalesce(nullif(trim(new.raw_user_meta_data ->> 'full_name'), ''), split_part(coalesce(new.email, 'Mico learner'), '@', 1)),
