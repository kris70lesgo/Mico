...
interface Props{organ:StudyOrgan;stage?:boolean;showCaption?:boolean;mode?:'study'|'identify'|'reveal';target?:string;onTargetSelect?:(id:string)=>void}

export default function StudyViewer({organ,stage=false,showCaption=stage,mode='study',target,onTargetSelect}:Props){
 ...
 useEffect(()=>{
  ...
  const loader=new GLTFLoader();loader.setMeshoptDecoder(MeshoptDecoder);loader.load(organ.model,gltf=>{if(disposed)return;model=gltf.scene;const box=new T.Box3().setFromObject(model),size=box.getSize(new T.Vector3()),center=box.getCenter(new T.Vector3()),scale=3.55/Math.max(size.x,size.y,size.z,.01);model.scale.setScalar(scale);model.position.copy(center.multiplyScalar(-scale));pivot.add(model);},undefined,()=>{if(!disposed)setError(true);});
 ...
 return <div className={`study-viewer ${stage?'study-stage':''} study-mode-${mode}`} style={{'--study-accent':organ.accent} as React.CSSProperties}>
  ...
  {organ.hotspots.map((hotspot,index)=><button ref={node=>{if(node)points.current[index]=node;}} key={hotspot.id} className={`study-hotspot ${selected===hotspot.id?'selected':''} ${mode==='reveal'&&target===hotspot.id?'revealed':''}`} style={{'--hotspot':hotspot.color} as React.CSSProperties} onClick={()=>choose(hotspot.id)} aria-label={`Select ${hotspot.label}`}><Crosshair size={stage?17:13}/></button>)}
